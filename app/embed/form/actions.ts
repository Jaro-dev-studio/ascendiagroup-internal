...
export async function submitEmbedForm({
  ...
  try {
    // Validate form data (use the refined schema for full validation)
    const validatedData = embedFormSchemaWithRefinement.parse(formData);
    ...
    const submission = await prisma.embedFormSubmission.create({
      data: {
        name: validatedData.name,
        email: validatedData.email,
        hasExistingCodebase: validatedData.hasExistingCodebase,
    ...
    // Skip CRM contact creation for low budget or mobile-only submissions
    if (isQualified) {
      const baseUrl = process.env.NODE_ENV === "development" ? "http://localhost:3000" : "https://studio.jaro.dev";
      console.log(`[FormSubmit] Triggering async processing for qualified submission ${submission.id}`);
      
      try {
        // Don't await the full response, just ensure the request is initiated
        ...
        console.log(`[FormSubmit] Processing request sent for submission ${submission.id}`);
      } catch (error) {
        ...
        await reportOpsFailure({
          ...
          summary: "Could not start processing for a qualified lead",
          error,
      ...
      // channel: a low-budget or mobile-only enquiry is a business decision to
      // make, not something to discover later in the submissions table.
      console.log(`[FormSubmit] Skipping CRM processing for unqualified submission ${submission.id} (lowBudget: ${showLowBudgetMessage})`);
