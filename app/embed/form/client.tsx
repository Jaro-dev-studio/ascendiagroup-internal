...
import { submitEmbedForm, validateEmail } from "./actions";
import { embedFormSchema, type EmbedFormData } from "./schema";

// Animated border component for form options
...
function AnimatedOption({ children, isSelected, onClick, onKeyDown, className = "" }: AnimatedOptionProps) {
  ...
  return (
    ...
    >
      ...
      {/* Animated border from left side */}
      {!isSelected && (
        <motion.div
          className="pointer-events-none absolute inset-0 rounded-lg border-[1.5px] border-[#8B8B8B]"
          initial={{ clipPath: "inset(0 100% 0 0)" }}
          animate={{ 
            clipPath: isHovered ? "inset(0 50% 0 0)" : "inset(0 100% 0 0)"
          }}
          transition={{ duration: 0.25, ease: "easeOut", delay: isHovered ? 0.1 : 0 }}
        />
      )}
      
      {/* Animated border from right side */}
      {!isSelected && (
        <motion.div
          className="pointer-events-none absolute inset-0 rounded-lg border-[1.5px] border-[#8B8B8B]"
          initial={{ clipPath: "inset(0 0 0 100%)" }}
          animate={{ 
            clipPath: isHovered ? "inset(0 0 0 50%)" : "inset(0 0 0 100%)"
          }}
          transition={{ duration: 0.25, ease: "easeOut", delay: isHovered ? 0.1 : 0 }}
        />
      )}
...
const FORM_STORAGE_KEY = "jaro-embed-form-data";
...
export function EmbedFormClient({
    ...
    switch (currentStep) {
      ...
      case 4: {
        ...
        if (needsOtherDescription) {
          return (
            hasServices && !!watchedValues.otherServiceDescription?.trim()
          );
        }
        return hasServices;
      }
  ...
  const goToNextStep = async () => {
    ...
    // Validate email with NeverBounce on step 2
    if (currentStep === 2) {
      ...
      try {
        ...
        if (result.data && !result.data.isValid) {
          ...
          // Show inline error based on validation result
  ...
  const getFieldsForStep = (step: number): (keyof EmbedFormData)[] => {
    switch (step) {
      case 1:
        return ["name"];
      case 2:
  ...
  const onSubmit = async (data: EmbedFormData) => {
    ...
    const result = await submitEmbedForm({
      formData: data,
      utmParams: {
        utmSource,
        utmMedium,
    ...
    if (result.data?.showLowBudgetMessage) {
      ...
      // Wait 2 seconds before redirecting to ensure backend processing starts
  ...
  // Skewed progress percentage - makes users feel further ahead
  ...
  // Low budget message screen
  if (showLowBudgetMessage) {
    const handleGoBack = () => {
      setShowLowBudgetMessage(false);
      setCurrentStep(3); // Go back to revenue selection step
    };
  ...
  return (
    <div className="relative flex h-screen flex-col bg-[#F7F7F7]">
      ...
      <form
        ...
        onKeyDown={(e) => {
          // Handle Enter key for multi-select step (step 8)
          ...
          <AnimatePresence mode="wait" custom={direction}>
            ...
            {currentStep === 4 && (
              ...
              >
                ...
                <p className="mb-8 text-[#2E2E2E]/50">
